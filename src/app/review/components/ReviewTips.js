      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {tips.map((tip) => (
            <div 
              key={tip.id} 
              className="bg-white rounded-2xl p-6 border border-[#1a365d]/20 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2"
            >
              <div className="flex items-start">
                <div className="w-12 h-12 bg-gradient-to-br from-[#1a365d] to-[#0249aa] rounded-lg flex items-center justify-center mr-4 shadow-md">
                  <div className="text-white">
                    {tip.icon}
                  </div>
                </div>
                <div>
                  <h3 className="text-xl font-bold text-[#1a365d] mb-2">{tip.title}</h3>
                  <p className="text-[#0249aa]">{tip.description}</p>
                </div>
              </div>
            </div>
          ))}
